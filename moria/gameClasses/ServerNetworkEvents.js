var ServerNetworkEvents = {
	...
	_onPlayerControlToTile: function (data, clientId) {
		...
		startTile = ige.$('foregroundMap').pointToTile(currentPosition.toIso());
		endTile = new IgePoint3d(parseInt(data[0]), parseInt(data[1]), 0);

		// // Generate a path to the destination tile and then start movement
		// // along the path
		// newPath = ige.server.pathFinder.generate(ige.server.collisionMap, startTile, endTile, function (tileData, tileX, tileY) {
		...
		playerEntity.path
			.clear()
			// .add(newPath) // Pathfinding not working @todo debug me
			.addPoints([startTile, endTile])
			.speed(5)
			.start();
	}
};
